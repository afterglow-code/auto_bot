...
def get_data_hybrid(code, start_date, end_date):
    ...
    try:
        ...
        if not df.empty:
            if isinstance(df.columns, pd.MultiIndex): return df['Close'][yf_code]
            return df['Close']
    except: pass
    return None
...
def get_todays_signal():
    ...
    try:
        ...
        try:
            ...
            print("⚠️ FDR 코스피 실패 -> YFinance 전환")
            kospi_df = yf.download('^KS11', start=start_date, end=end_date, progress=False)
            kospi = kospi_df['Close']['^KS11'].ffill() if isinstance(kospi_df.columns, pd.MultiIndex) else kospi_df['Close']

        # ETF 개별 종목
        df_list = []
        total = len(etf_tickers)
        for i, (name, code) in enumerate(etf_tickers.items()):
            print(f"\r[{i+1}/{total}] {name} 수집 중...", end='')
